...
func create_particle():
	...
	ps.body_set_param(water_col,PhysicsServer2D.BODY_PARAM_FRICTION,1)
	ps.body_set_param(water_col,PhysicsServer2D.BODY_PARAM_MASS,0.05)
	ps.body_set_param(water_col,PhysicsServer2D.BODY_PARAM_GRAVITY_SCALE,0.2)
	...
	vs.canvas_item_set_use_parent_material(water_particle,true)
